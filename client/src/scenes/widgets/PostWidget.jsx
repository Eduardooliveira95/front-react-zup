...
const PostWidget = ({ }) => {
  ...
  const handleModalSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://44.219.155.152:8080/noticia/rss/info",
        data
      );
  ...
  const getPost = async () => {
    try {
      const response = await axios.get(
        "https://44.219.155.152:8080/admin/filaPilha/noticias"
      );
  ...
  const patchLike = async (id) => {
    try {
      const response = await axios.post(
        `https://44.219.155.152:8080/noticia/likes/${idUser}/${id}`,
        {
          likes: 1
        }
        ,
        ...
        <WidgetWrapper key={item.id} m="2rem 0">
          ...
          {comentarioId === item.id && (
            <Box mt="0.5rem">
              ...
              <Box key={item.id}>
                <Divider />
                {item?.comentario.map((comentario, index) => {
