...
const AdminWidget = () => {
  ...
  const handleImportFile = async () => {
    ...
    input.onchange = async (event) => {
      ...
      try {
        await axios.post('https://44.219.155.152:8443/admin/importacao/txt', formData);
  ...
  const getUser = async () => {
    try {
      const response = await axios.get("https://44.219.155.152:8443/admin");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://44.219.155.152:8443/usuario/${id}`);
  ...
  const handleGenerateFile = async () => {
    ...
    try {
      const response = await axios.get(`https://44.219.155.152:8443/admin/csv/${fileName}`, {
        filename: fileName,
      }, {
        responseType: 'blob',
      });
  ...
  const handleSwitchChange = async (id, isActive) => {
    try {
      const response = await axios.put(`https://44.219.155.152:8443/admin/influencer/comum/${id}/${!isActive}`);
